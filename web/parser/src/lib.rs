...
use shared::{JsMove, JsPlayer, Move, Packet, PacketType, Player};
...
pub fn decode_bytes(bytes: &[u8]) -> Result<JsValue, JsValue> {
    ...
    match opcode {
        Some(code) => {
            match PacketType::from_u8(*code) {
                Some(PacketType::Spawn) => {
                    let data = borsh::from_slice::<Player>(&bytes[1..]).map_err(|e| JsValue::from_str(format!("error decoding player {}", e).as_str()))?;

                    Ok(serde_wasm_bindgen::to_value(&data).map_err(|e| JsValue::from_str(&e.to_string()))?)
                },
                Some(PacketType::Move) => {
                    let data = borsh::from_slice::<Move>(&bytes[1..]).map_err(|e| JsValue::from_str(format!("error decoding move {}", e).as_str()))?;
...
pub fn encode_into_bytes(packet: JsValue, opcode: u8) -> Result<Box<[u8]>, JsValue> {
    ...
    match opcode {
        ...
        2 => {
            let js_move: JsMove = serde_wasm_bindgen::from_value(packet).map_err(|x| JsValue::from_str(&x.to_string()))?;
